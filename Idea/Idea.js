...
//					__kendo_devtools_id: { hidden: true }
	...
	function getIdeaOnSuccess(sender, args){
		...
		while (projEnumerator.moveNext()) {
			...
			switch(proj.get_item("ID")){
				case itemId:
					...
					var fechaCreacion = $("#fechaCreacion").data("kendoDatePicker");
					fechaCreacion.value(proj.get_item("Created"));
					fechaCreacion.readonly();
					setPeoplePickerDefaultValuePatrocinador(proj.get_item("Patrocinador").get_lookupValue());
					setPeoplePickerDefaultValueCliente(proj.get_item("Cliente").get_lookupValue());					
					setPeoplePickerDefaultValueGPortafolio(proj.get_item("GestorDePortafolio").get_lookupValue());
					setPeoplePickerDefaultValueGDemanda(proj.get_item("GestorDeDemanda").get_lookupValue());
					setPeoplePickerDefaultValueEvaluador(proj.get_item("Evaluador").get_lookupValue());
	...
	function guardarFormulario(){
		currList = context.get_web().get_lists().getByTitle("Idea");
		if (currentItem == null){
		...
		oListItem.set_item("Title", nombreProyecto);
		var unidadSolicitante = $("#selectUnidadSolicitante").data("kendoDropDownList");
		oListItem.set_item("unidadSolicitante", unidadSolicitante.value());	
		var empresa = $("#selectEmpresa").data("kendoDropDownList");
	...
	function guardarIdeaOnSucceeded(sender, request) {
		...
		$("#dialog").kendoDialog({width: "400px", title: "Cloud EPM", closable: true, modal: true,
